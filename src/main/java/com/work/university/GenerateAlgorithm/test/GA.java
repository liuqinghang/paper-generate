...
import com.work.university.tools.utils.SpringBeanUtils;
...
public class GA {
    ...
    // 进化种群
    public static Population evolvePopulation(Population pop, RuleBean rule,QuestionService questionService) {
        ...
        for (int i = elitismOffset; i < newPopulation.getLength(); i++) {
            ...
            // 交叉
            Paper child = crossover(parent1, parent2, rule, questionService);
            child.setId(i);
        ...
        for (int i = elitismOffset; i < newPopulation.getLength(); i++) {
            ...
            mutate(tmpPaper,questionService);
            // 计算知识点覆盖率与适应度
     ...
     * @return
     */
    public static Paper crossover(Paper parent1, Paper parent2, RuleBean rule, QuestionService questionService) {
     ...
     * @param paper
     */
    public static void mutate(Paper paper, QuestionService questionService) {
        ...
        for (int i = 0; i < paper.getQuestionSize(); i++) {
            if (Math.random() < mutationRate) {
                ...
                list = Arrays.asList(questionService.getQuestionListWithOutSId(tmpQuestion));
                if (list.size() > 0) {
                    // 随机获取一道
